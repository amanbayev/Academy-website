...
export function ProgramCard({program, locale, hrefLabel}: {program: Program; locale: Locale; hrefLabel: string}) {
  return (
    <article className="overflow-hidden rounded-lg border border-academy-border bg-white shadow-[0_8px_26px_rgba(17,24,39,0.035)] transition hover:-translate-y-1 hover:shadow-soft">
      <div className="relative h-40">
        <Image src={program.image} alt="" fill className="object-cover" />
      </div>
      <div className="p-5">
        <h3 className="min-h-14 text-base font-bold leading-6">{program.title[locale]}</h3>
        <p className="mt-3 line-clamp-2 text-sm leading-6 text-academy-text">{program.shortDescription[locale]}</p>
        <div className="mt-4 grid gap-2 text-[13px] text-academy-text">
